...
outgroup_vcf="${1:-${analysis_dir}/work/dsuite/outgroup/DH01.known_sites.vcf.gz}"
work_root="${analysis_dir}/work/dsuite"
mkdir -p "${work_root}"
...
for depth in 3 5 10; do
  ...
  python3 "${analysis_dir}/scripts/11_merge_qglauca_outgroup.py" \
    ...
    --coordinate-map "${analysis_dir}/config/dsuite/known_sites.coordinate_map.tsv" \
